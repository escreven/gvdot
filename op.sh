...
#
# op.sh - A build and deployment script for gvdot.
#
...
function wheel_file {
    echo "dist/gvdot-$1-py3-none-any.whl"
}

function sdist_file {
    echo "dist/gvdot-$1.tar.gz"
}

#
# Succeed iff all version markers in the project are identical (currently in
# pyproject.toml and gvdot.py.)  Output is the consistent version.
#
...
function build_dist {
    ...
    if [[ -d src/gvdot.egg-info ]]; then
        echo "Deleting egg-info"
        /bin/rm -r -f src/gvdot.egg-info
    fi
}
